...
char	*truncate_zeros(char *untrimmed)
{
	...
	int zero_n = 0;
	while (untrimmed[zero_n] && untrimmed[zero_n] == '0')
		zero_n++;
	
	int 	trimmedlen = ft_strlen(untrimmed) - zero_n;
	char	*trimmed = malloc(sizeof(char) * trimmedlen + 1);
	trimmed[trimmedlen] = '\0';

	for (int i = 0; i < trimmedlen; i++)
		trimmed[i] = untrimmed[i + zero_n];
